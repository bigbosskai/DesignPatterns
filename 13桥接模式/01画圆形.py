...
3、一个类存在两个独立变化的维度(下例是颜色和坐标的变化)，且这两个维度都需要进行扩展。
...
class RedCircle(DrawAPI):
    def drawCircle(self, radius, x, y):
        print("Drawing Circle color: red, radius: {}\t{}\t{}".format(radius, x, y))


class GreenCircle(DrawAPI):
    def drawCircle(self, radius, x, y):
        print("Drawing Circle color: green, radius: {}\t{}\t{}".format(radius, x, y))
...
class Circle(Shape):
    def __init__(self, radius, x, y, drawapi):
...
if __name__ == '__main__':
    red = Circle(2.5, 0, 0, RedCircle())
    green = Circle(1.2, 10, 10, GreenCircle())
    red.draw()
    green.draw()
